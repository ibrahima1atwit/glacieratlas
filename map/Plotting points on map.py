...
for index, row in twila_subset.iterrows():
    ...
    #for index1, row1 in names_subset.iterrows():
...
# Add markers for points in csv file with the names
# for index, row in subset2.iterrows():
#     # Get the longitude and latitude values
#     latitude = row['LAT']
#     longitude = row['LONG']
#     name = row['NAMES']
#     folium.Marker([latitude,longitude], popup=name, icon=folium.Icon(color='red')).add_to(my_map)
